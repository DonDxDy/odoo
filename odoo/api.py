...
def ondelete(*, at_uninstall):
    ...
    The goal of this decorator is to allow client-side errors when unlinking records if from
    a business point of view it does not make sense to delete such records.

    E.g. a user that tries to delete a validated sales order.

    While this could be done by simply overriding the ``unlink`` method of the model, it has
    the drawback of not being compatible with module uninstallation. When uninstalling the module
    the unlink override could raise but we don't care because the module is being uninstalled
    thus **all** records related to the module should be removed anyway.

    This means that by overriding unlink instead, there's a big chance that some tables/records
    may remain as leftover data from the uninstalled module which leaves the schema in an
    inconsistent state, plus there's the risk of conflict if the module is ever reinstalled.

    Methods decorated with ``@ondelete`` should raise an error following some conditions, and
    by convention, the method should be named either ``_unlink_if_<condition>`` or
    ``_unlink_except_<not_condition>``.
    ...
    :param bool at_uninstall: Whether the decorated method should be called if the module that
        implements said method is being uninstalled. Should almost always be ``False``.

    .. danger::
        The parameter ``at_uninstall`` should only be set to ``True`` if the check you're
        implementing also applies when uninstalling the module.

        E.g. it doesn't matter if when uninstalling ``sale``, validated sales orders are being
        deleted because all data pertaining to ``sale`` should be deleted anyway, in that case
        ``at_uninstall`` should be set to ``False``

        However it does make sense to prevent the removal of the default language if no other
        languages are installed, since it will break a lot of basic behaviour if the default
        language is deleted, in this case ``at_uninstall`` should be set to ``True``.
    """

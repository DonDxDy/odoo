...
class ResConfigSettings(models.TransientModel, ResConfigModuleInstallationMixin):
    ...
    def onchange_module(self, field_value, module_name):
        module = self.env['ir.module.module']._get(module_name[7:])
        if not int(field_value) and module.state in ('to install', 'installed', 'to upgrade'):
            deps = module.sudo().downstream_dependencies()
            dep_names = (deps | module).mapped('shortdesc')
            message = '\n'.join(dep_names)
            return {
                'warning': {
                    'title': _('Warning!'),
    ...
    def set_values(self):
        ...
        for name, model, field in classified['default']:
            if isinstance(self[name], models.BaseModel):
                ...
                value = self[name]
            if value != current_settings[name]:

...
var SnippetEditor = Widget.extend({
    ...
    removeSnippet: async function () {
        ...
        await this.wysiwyg.withMutation($('#wrapwrap'), removeSnippet);
    },
    /**
    ...
    clone: async function () {
        ...
        this.wysiwyg.withMutation(this.$snippetBlock.parent(), async () => {
    ...
    _onDragAndDropStop: async function (ev, ui) {
        ...
        if (this.dropped) {
            await this.wysiwyg.withMutation($parent, () => {
                if (prev) {
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        this.$document.on('click.snippets_menu', '.o_default_snippet_text', (ev) => {
            ...
            const autoSelectDefaultText = async (context) => {
                ...
                this.wysiwyg.withMutation($defaultSnippetText, () => $defaultSnippetText.removeClass('o_default_snippet_text'))
            };
            ...
            jQueryDraggableOptions: {
                ...
                stop: async function (ev, ui) {
                    ...
                    if (dropped) {
                        ...
                        await self.wysiwyg.withMutation($snippetToInsert.parent(), () => {
                            if (prev) {
    ...
    async _removeLastSnippetActivated() {
        ...
        if ($lastSnippet.length) {
            await this.wysiwyg.withMutation($lastSnippet, () => $lastSnippet.removeClass(classesToRemove));
        }
    },
});

...
class AccountEdiDocument(models.Model):
    ...
    state = fields.Selection([('to_send', 'Sending'), ('sent', 'Sent'), ('to_cancel', 'Cancelling'), ('cancelled', 'Cancelled')])
    error = fields.Html()
    ...
    def _prepare_jobs(self):
        ...
        * documents:      The documents related to this job. If edi_format_id does not support batch, length is one
        ...
        for edi_doc in docs:
            ...
            if not to_process.get(key, None):
                to_process[key] = self.env['account.edi.document']
            ...
            for document in documents:
                ...
                if move_result.get('attachment'):
                    ...
                    if 'blocked_level' in account_edi_document._fields:
                        values['blocked_level'] = move_result.get('blocked_level', DEFAULT_BLOCKING_LEVEL if 'error' in move_result else False)
                    ...
                    if 'blocked_level' in account_edi_document._fields:
                        document.blocked_level = move_result.get('blocked_level', DEFAULT_BLOCKING_LEVEL) if document.error else False
    ...
    def _process_documents_no_web_services(self):
        ...
        """
        jobs = self.filtered(lambda d: not d.edi_format_id._needs_web_services())._prepare_jobs() # TODO trier payment et invoice + rename process_jobs
        for docs, doc_type in jobs:
            self._process_job(docs, doc_type)

    def _process_documents_web_services(self, job_count=None, with_commit=False):

...
class AccountChartTemplate(models.Model):
    ...
    def _get_coa_record_fields(self, template_model, target_model, excluded_fields=[]):
        ''' Helper used to retrieve all ir.model.fields to be used on the targeted model to create the record based on
        its template.
    ...
    @api.model
    def _prepare_coa_record_field_value(self, company, template, orm_field, loaded_data):
        ''' Prepare the value to create the record from a single template field.

        :param company:     The company on which the chart template is currently loading.
        ...
        if orm_field.type == 'many2many' and orm_field.comodel_name in loaded_data:
            ...
            return [(6, 0, [loaded_records[record] for record in template[orm_field.name]])]
            ...
            sub_fields = self._get_coa_record_fields(template[orm_field.name], target_model, excluded_fields=[orm_field.inverse_name])
    ...
    @api.model
    def _prepare_coa_record_create_vals(self, company, template, target_model, orm_fields, loaded_data):
        ...
        for field in orm_fields:
            ...
            if field.type in ('many2one', 'many2many')\
                ...
                vals[field.name] = self._prepare_coa_record_field_value(company, template, field, loaded_data)
    ...
    @api.model
    def _prepare_coa_record_update_vals(self, company, template, orm_fields, loaded_data):
        ...
        '''
        return {field.name: self._prepare_coa_record_field_value(company, template, field, loaded_data) for field in orm_fields}
    ...
    def _create_coa_records_with_xml_ids(self, company, templates, target_model, create_vals_list):
        ...
        :param target_model:        The model loaded by the template.
        :param create_vals_list:    The list a values created by the '_prepare_coa_record_create_vals' method.
        :return:                    A recordset.
        '''
    ...
    def _load_templates(self, company):
        ''' Load the templates from the current chart template to the company passed as parameter.
        ...
        for template_model_name, data in to_load:
            ...
            for template in templates:
                ...
                if template_model_name == 'account.account.template':
                ...
                elif template_model_name == 'account.tax.template':
                    ...
                    for rep_line_command, template_rep_line in zip(rep_line_commands, template_rep_lines):
                        ...
                        rep_line_vals['tag_ids'] = [(6, 0, tags.ids)]
        ...
        for template, record, delayed_fields in records_to_post_fix:
            record.write(self._prepare_coa_record_update_vals(company, template, delayed_fields, loaded_data))

        return loaded_data

    @api.model
    def _prepare_liquidity_journals(self, company, loaded_data):
        ''' Prepare the dictionaries to be used to create the bank/cash journals.
    ...
    def _load_coa(self, company):
        ''' Load the chart template for the company passed as parameter.
        ...
        loaded_data = self._load_templates(company)
        ...
        # Journals.
        to_write_on_company = {}
        journal_vals_list = self._prepare_journals(company, loaded_data) + self._prepare_liquidity_journals(company, loaded_data)
        for journal in self.env['account.journal'].create(journal_vals_list):

...
QUnit.module('fields', {}, function () {
    ...
    QUnit.module('relational_fields', {
        ...
        QUnit.test('one2many kanban (editable): properly handle create_text node attribute', async function (assert) {
            assert.expect(1);
            ...
            var form = await createView({
                ...
                data: this.data,
                arch: '<form string="Partners">' +
                    '<field name="turtles" create_text="Add turtle" mode="kanban">' +
                    '<kanban>' +
                    '<templates>' +
                    '<t t-name="kanban-box">' +

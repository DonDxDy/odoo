...
function addLegacyMockEnvironment(comp: Component<{}, OdooEnv>, baseConfig: TestConfig, legacyParams: LegacyMockParams = {}) {
  const cleanUps: (() => void)[] = [];
...
function beforeEachActionManager(): TestConfig {
  ...
  return { serverData, services, browser };
}
...
QUnit.module("web client integrated tests", (hooks) => {
  hooks.beforeEach(() => {
    ...
    baseConfig.actions = actionsRegistry;
  });
...
QUnit.module("Action Manager Legacy Tests Porting", (hooks) => {
  ...
  QUnit.skip("switch request to unknown view type", async function (assert) {
    ...
    assert.verifySteps(["/wowl/load_menus", "/web/action/load", "load_views", "/web/dataset/search_read"]);
  ...
  QUnit.skip("execute action from dirty, new record, and come back", async function (assert) {
    ...
    assert.strictEqual($(webClient.el!).find(".o_control_panel .breadcrumb-item").text(), "PartnersNew");

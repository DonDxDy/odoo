...
export const YEAR_OPTIONS: { [optionId: string]: object } = {
  this_year: {
    id: "this_year",
    groupNumber: 2,
    format: "YYYY",
...
export function getOptionsWithDescriptions(OPTIONS: {
   [id: string]: { id: string; groupNumber?: number; description?: Stringifiable | string; [key: string]: any }
}) {
    const options = [];
    for (const option of Object.values(OPTIONS)) {
        const { id, groupNumber, description, } = option;
        const res: { id: string; groupNumber?: number; description?: string; } = { id, };
        if (description) {
            res.description = description.toString();
        }
        if (groupNumber) {
...
export function sortPeriodOptions(
  options: { granularity: string; setParam: { [key: string]: number } }[]
) {
  options.sort((o1, o2) => {
 ...
 * @returns {boolean}
 */
export function yearSelected(selectedOptionIds: string[]) {
...
export function assembleDomains(domains: string[], operator: "AND" | "OR"): string {
  // make something!!!
  if (domains.length > 1) {
    throw Error();

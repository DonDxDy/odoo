...
QUnit.test('add an emoji', async function (assert) {
    ...
    const env = await this.start();
    const composer = env.invoke('Composer/create');
    await this.createComposerComponent(composer);
    await afterNextRender(() =>
        document.querySelector('.o-Composer-buttonEmojis').click()
    );
    await afterNextRender(() =>
        document.querySelector('.o-EmojisPopover-emoji[data-unicode="😊"]').click()
    );
    assert.strictEqual(
...
QUnit.test("channel with no commands should not prompt any command suggestions on typing /", async function (assert) {
    ...
    await afterNextRender(() => {
        const composer_text_input = document.querySelector('.o-ComposerTextInput-textarea');
...
QUnit.test('composer: paste attachments', async function (assert) {
    ...
    await afterNextRender(() =>
        pasteFiles(document.querySelector('.o-ComposerTextInput'), files)
    );
    assert.containsOnce(
        document.body,
        `
...
QUnit.test('composer with thread typing notification status', async function (assert) {
    ...
    await this.createComposerComponent(
        thread.$$$composer(),

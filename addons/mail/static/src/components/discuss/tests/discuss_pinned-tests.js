odoo.define('mail/static/src/components/discuss/tests/discuss_pinned-tests.js', function (require) {
'use strict';
...
QUnit.module('discuss_pinned-tests.js', {
    beforeEach() {
        beforeEach(this);
...
QUnit.test('sidebar: pinned channel 2: open pinned channel', async function (assert) {
    ...
    const env = await this.start();
    const threadGeneral = env.invoke('Thread/findFromId', {
        $$$id: 20,
        $$$model: 'mail.channel',
    });
...
QUnit.test('sidebar: unpin channel from bus', async function (assert) {
    ...
    assert.containsOnce(
        document.body,
        `.o-Discuss-thread[data-thread-local-id="${env.messaging.$$$inbox().localId}"]`,
        "the Inbox is opened in discuss"
    );
    assert.containsOnce(
        document.body,
    ...
    await afterNextRender(() => {
        const notif = [
            ["dbName", 'res.partner', env.messaging.$$$currentPartner().$$$id()],
            {

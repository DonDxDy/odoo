odoo.define('mail/static/src/components/discuss/tests/discuss_moderation-tests.js', function (require) {
'use strict';
...
QUnit.module('discuss_moderation-tests.js', {
    beforeEach() {
        beforeEach(this);
...
QUnit.test('as moderator, moderated channel with pending moderation message', async function (assert) {
    ...
    await afterNextRender(() =>
        document.querySelector('.o-Message-checkbox').click()
    );
    assert.ok(
        document.querySelector('.o-Message-checkbox').checked,
    ...
    await afterNextRender(() =>
        document.querySelector('.o-widget-Discuss-controlPanelButtonModeration.o-discard').click()
    );
    assert.containsOnce(
        document.body,
...
QUnit.test('as moderator, pending moderation message accessibility', async function (assert) {
    ...
    const message = env.invoke('Message/findFromId', { $$$id: 100 });
    assert.containsOnce(
        document.body,
...
QUnit.test('as moderator, new pending moderation message posted by someone else', async function (assert) {
    ...
    await afterNextRender(() => {
        const notifications = [[
            ['my-db', 'res.partner', env.messaging.$$$currentPartner().$$$id()],
            { type: 'moderator', message: messageData },
        ]];
        this.widget.call('bus_service', 'trigger', 'notification', notifications);
    });
    const message = env.invoke('Message/findFromId', { $$$id: 1 });
    assert.containsOnce(
        document.body,
        `.o-Message[data-message-local-id="${message.localId}"]`,
        "the pending moderation message should be in the channel"

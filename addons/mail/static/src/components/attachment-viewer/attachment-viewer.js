odoo.define('mail/static/src/components/attachment-viewer/attachment-viewer.js', function (require) {
'use strict';
...
class AttachmentViewer extends usingModels(Component) {
    ...
    mounted() {
        ...
        this._renderedAttachment = this.record.$$$attachment(this);
     ...
     */
    _close() {
        this.env.invoke('AttachmentViewer/close', this.record);
    }
    ...
    _handleImageLoad() {
        if (
            ...
            this.env.invoke('Record/update', this.record, {
                $$$isImageLoading: true,
            });
    ...
    _next() {
        const attachmentViewer = this.record;
        const index = attachmentViewer.$$$attachments(this).findIndex(attachment =>
            attachment === attachmentViewer.$$$attachment(this)
        );
        const nextIndex = (index + 1) % attachmentViewer.$$$attachments(this).length;
        this.env.invoke('Record/update', attachmentViewer, {
            $$$attachment: link(attachmentViewer.$$$attachments(this)[nextIndex]),
        });
    }
    ...
    _updateZoomerStyle() {
        ...
        const tx = (
            (
                this._imageRef.el.offsetWidth * attachmentViewer.$$$scale(this) >
        ...
        const ty = (
            (
                this._imageRef.el.offsetHeight * attachmentViewer.$$$scale(this) >

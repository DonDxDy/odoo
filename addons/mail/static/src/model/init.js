odoo.define('mail/static/src/model/init.js', function (require) {
'use strict';

const {
    registry,
} = require('mail/static/src/model/core.js');
const ModelField = require('mail/static/src/model/model-field.js');
const {
...
const actions = defineActions({
     ...
     * extensions. This launches the generation of actions and models.
     *
     * @param {Object} param0
    ...
    'init/start'(
        ...
        env.actions = new Map();
        env.invoke('init/_makeActions');
        env.invoke('init/_makeModels');
        /**
         * Check that declared model fields are correct.
         */
        env.invoke('init/_checkDeclaredFieldsOnModels');
        /**
            ...
            for (const patch of generatable.patches) {
                switch (patch.type) {
                    ...
                    case 'instance':
                        ...
                        break;
                    case 'field':
                        env.invoke('init/_applyModelPatchFields', model, patch.patch);
                        break;
                }
            }
        ...
        for (const model of env.models.values()) {
            ...
            for (const modelField of model.fields.values()) {
                model.prototype[modelField.name] = function (ctx) {
                    ...
                    const field = env.invoke('RecordField/get', fieldLocalId);
                    return env.invoke('RecordField/read', field, ctx);
                };

odoo.define('mail/static/src/widgets/discuss-partner-invite-dialog/discuss-partner-invite-dialog.js', function (require) {
'use strict';
...
const PartnerInviteDialog = Dialog.extend({
    ...
    init(parent, { activeThread, messagingEnv }) {
        ...
        this.env = env;
        this._super(parent, {
            title: _.str.sprintf(this.env._t("Invite people to #%s"), activeThread.$$$displayName()),
            size: 'medium',
            buttons: [{
        ...
        this.$input.select2({
            ...
            formatResult: item => {
                ...
                if (item.id === 'odoobot') {
                    ...
                    const partner = this.env.invoke('Partner/findFromId', {
                        $$$id: item.id,
                    });

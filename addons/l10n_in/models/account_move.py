...
class AccountMoveLine(models.Model):
    ...
    def _compute_invoice_line_taxes_by_group(self):
        ...
        # line.tax_amount_by_group = {'SGST': 9.0, 'CGST': 9.0, 'Cess': 2.0}
        for line in self:
            taxes = dict()
            for ln in self.search([('base_line_ref','=',str(line.id)), ('tax_tag_ids','!=',False), ('move_id','=',line.move_id.id)]):
                for tag in ln.tax_tag_ids:
                    tag_name = tag.name and tag.name.replace('+','').replace('-','') or 'Ungroupd'
                    tag_name = tag_name and tag_name.upper()
                    taxes.setdefault(tag_name, 0.0)
                    taxes[tag_name] += ln.credit and ln.credit or ln.debit

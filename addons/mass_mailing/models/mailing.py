...
class MassMailing(models.Model):
    ...
    def _action_send_statistics(self):
        ...
        for mailing in self:
            ...
            mail_values = {
                'subject':  _('24H Stats of %s "%s"') %
                             (self._get_pretty_mailing_type(), mailing.subject),
    ...
    def _prepare_statistics_email_values(self):
        ...
        if self.mailing_type == 'mail':
            kpi = {
                'kpi_fullname': _('Engagement on %i %s Sent') % (self.expected, mailing_type),
                'kpi_col1': {
        ...
        return {
            'title': _('24H Stats of %s "%s"') % (mailing_type, self.subject),
            'top_button_label': _('More Info'),
            'top_button_url': url_join(web_base_url, f'/web#id={self.id}&model=mailing.mailing&view_type=form'),
            'kpi_data': [
                kpi,
                {
                    'kpi_fullname': _('Business Benefits on %i %s Sent') % (self.expected, mailing_type),
                    'kpi_action': None,
                    'kpi_col1': {},
                    'kpi_col2': {},
                    'kpi_col3': {},

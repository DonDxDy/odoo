...
const CountdownWidget = publicWidget.Widget.extend({
    ...
    _render: function () {
        ...
        // Todo: This should be remove once JWEditor bug fixed.
        // JWEditor update reference variable while updating dom when we remove snippet.
        // So In that case canvas is removed from this.diff variable
        // which leads to generate traceback in existing code.
        const canvas = this.diff.filter(e => e.canvas && e.canvas.querySelector('canvas'));
        if (!canvas.length) {
            clearInterval(this.setInterval);
            return;
        }
